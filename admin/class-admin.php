...
//
//
//			register_post_meta('', '_leira-access', [
//				'show_in_rest'      => true,
//				'single'            => true,
//				'type'              => 'string',
...
//
//			// Enqueue script to add sidebar access control for posts, pages and custom post types
//			wp_enqueue_script(
//				'leira-access-sidebar',
//				plugin_dir_url(__DIR__).'/build/sidebar.js',
...
//		add_action('enqueue_block_editor_assets', 'sidebar_plugin_script_enqueue');
	...
	/**
	 * Register meta for Gutenberg sidebar
	 *
	 * @since  1.1.0
	 * @access public
	 */
	public function admin_init()
	{
		...
		register_post_meta('', '_leira-access-roles', [
			//'show_in_rest'      => true,
			'single'            => true,
			'type'              => 'array',
//			'items'             => [
...
//			],
			...
			'sanitize_callback' => function ($value){
				// keep only role keys
				//TODO: Check the role is valid, and remove duplicates
	...
	/**
	 * Enqueue the admin sidebar script
	 *
	 * @since  1.1.0
	 * @access public
	 */
	public function enqueue_block_editor_assets()
	{
		...
		$assets = include plugin_dir_path(__DIR__).'build/sidebar.asset.php';
		// Enqueue script to add sidebar access control for posts, pages and custom post types
		wp_enqueue_script(
			'leira-access-sidebar',
			plugin_dir_url(__DIR__).'build/sidebar.js',
...
//		wp_enqueue_style($this->plugin_name, plugin_dir_url(__FILE__).'css/leira-access-admin.css', array(),$this->version, 'all');
	...
	public function enqueue_scripts()
	{
		 ...
		 *
		 * An instance of this class should be passed to the run() function
		 * defined in Leira_Access_Loader as all the hooks are defined
		 * in that particular class.
		 *
		 * The Leira_Access_Loader will then create the relationship
		 * between the defined hooks and the functions defined in this
		...
		wp_enqueue_script( 'leira-access', plugin_dir_url( __FILE__ ) . 'js/leira-access-admin.js', array( 'jquery' ), $this->version, false );
		...
		wp_localize_script('jquery', 'leiraAccessData', ['roles' => $roles]);
	}
			...
			<div class="leira-access-controls">
				...
				<div class="leira-access-roles">
					...
					foreach ($display_roles as $role => $name) {
						...
						$checked       = checked(true, (is_array($checked_roles) && in_array($role, $checked_roles)),
							false);
	...
	public function save($id, $type = 'post', $use_post_id = true)
	{
		...
		if (!isset($_POST['leira-access-nonce']) || !wp_verify_nonce($_POST['leira-access-nonce'],
				'leira-access-nonce-name')) {
			return false;
		}
		...
		if (!empty($status)) {
			...
			if ($status == 'in' && !empty ($roles)) {
				...
				if (in_array($status, array('in', 'out'))) {
					$saved_data = $status;
				}
			}
		}
		...
		switch ($type) {
			case 'post':
				if ($saved_data) {
					update_post_meta($id, Plugin::META_KEY, $saved_data);
				}else {
			...
			case 'term':
				if ($saved_data) {
					update_term_meta($id, Plugin::META_KEY, $saved_data);
				}else {
					delete_term_meta($id, Plugin::META_KEY);
				}
				break;
			case 'widget':
				return $saved_data;
